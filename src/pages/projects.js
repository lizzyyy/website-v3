...
const ProjectsPage = ({ location }) => {
  ...
  useEffect(() => {
    if (location?.state?.firstVisit != null) {
      setGm(location.state.firstVisit);
    }
  }, [location]);
  ...
  useEffect(() => {
    if (project != "" && projectRef.current && window.innerWidth < 767) {
      setTimeout(()=> {
            ...
            <div className={s.splitScreen}>
              <div className={s.leftScreen}>
                ...
                <div className={s.projectList}>
                  <div onClick={() => setProject("homepage")} className={project == "homepage" ? s.activeProject : ""}>
                    <Project title="SOCKET MOBILE HOMEPAGE" company="Socket Mobile, Inc." year="2025" fav={true}
                            desc="Redesigning the company's public facing website" />
                  </div>
                  <div onClick={() => setProject("apple")} className={project == "apple" ? s.activeProject : ""}>
                    <Project title="SOCKET MOBILE x APPLE" company="Socket Mobile, Inc." year="2025" fav={true}
                            desc="Creating a no-frills landing page to promote a budding partnership between the two companies" />
                  </div>
                  <div onClick={() => setProject("xr")} className={project == "xr" ? s.activeProject : ""}>
                  ...
                  <div onClick={() => setProject("socketcare")} className={project == "socketcare" ? s.activeProject : ""}>
                    <Project title="SOCKETCARE" company="Socket Mobile, Inc." year="2023" fav={false}
                            desc="Reworking the customer-facing warranty purchase and product return system" />
                  </div>
                  <div onClick={() => setProject("innod")} className={project == "innod" ? s.activeProject : ""}>
                  ...
                  <div onClick={() => setProject("circles")} className={project == "circles" ? s.activeProject : ""}>
                  ...
                  <div onClick={() => setProject("gitlet")} className={project == "gitlet" ? s.activeProject : ""}>
                    <Project title="GITLET" company="Project" year="2021" fav={false}
                            desc="A local version control system mimicking git" />
                  </div>
                  <div onClick={() => setProject("press&go")} className={project == "press&go" ? s.activeProject : ""}>
                    <Project title="PRESS&GO" company="Case Study - Designing for UX" year="2021" fav={false}
                            desc="Creating a beginner friendly clothing alteration tool to minimize fabric waste" />
                  </div>
                </div>
              </div>

              <div className={s.rightScreen} ref={projectRef}>
                <Clock />
                {project == "" ? (
                  <p className={s.noProject}>What would you like to know?</p>
                ) : ""}
                {project == "homepage" ? <ProjectItem img1={homepage2} img2={homepage1}
                                                        desc="Lead the redesign of a visually engaging and conversion-focused homepage for the web store that helps shoppers feel confident making a purchase without feeling overwhelmed."
                                                        design="Powerpoint, Figma"
                                                        stack="HTML, JavaScript (jQuery), CSS (SCSS)"
                                                        more="/projects/sm-home/"
                                                        link="https://www.socketmobile.com"
                                                        github="" /> : ""}
                {project == "apple" ? <ProjectItem img1={apple1} img2={apple2}
                                                        desc="Design and implement a clean, intuitive landing page for Socket Mobile's collaboration with Apple, to guide users towards informed purchasing decisions with minimal friction."
                                                        design="Figma"
                                                        stack="HTML, JavaScript (jQuery), CSS (SCSS), C# (ASP.NET)"
                                                        more="/projects/sm-apple/"
                                                        link="https://www.socketmobile.com/apple"
                                                        github="" /> : ""}
                ...
                {project == "socketcare" ? <ProjectItem img1={socketcare1} img2={socketcare2}
                                                        ...
                                                        design="Figma"
                                                        stack="HTML, JavaScript (jQuery), CSS (SCSS), C# (ASP.NET)"
                                                        more="/projects/socketcare/"
                                                        link="https://www.socketmobile.com/support/warranty-registration"
                                                        github="" /> : ""}
